...
Note that 64-word EEPROMs are not supported for unknown reasons.  This may be related to the funny fixed word at address `0x45`.  Since this is in the same address range as the FT232R uses for factory-programmed data, it is somewhat likely that it is an accidentally merged bit of FT232R circuitry.
...
- word 0x00:
  ...
  - bit 8: FT1248 clock polarity (CPOL)
    - 0: low
    - 1: high
  - bit 9: FT1248 bit order
    - 0: MSB
    - 1: LSB
  - bit 10: FT1248 flow control disabled
